...
COMPETITOR_ACCOUNTS = ['capcutdailyuk']
...
def calculate_metrics(df):
    ...
    # Get counts
    df['shareCount'] = pd.to_numeric(df.get('shareCount', 0), errors='coerce').fillna(0)
    df['diggCount'] = pd.to_numeric(df.get('diggCount', 0), errors='coerce').fillna(0)
    df['playCount'] = pd.to_numeric(df.get('playCount', 0), errors='coerce').fillna(0)
...
def create_video_sheet(ws, df):
    ...
    for idx, (_, row) in enumerate(df.iterrows(), 2):
        ...
        ws.cell(row=idx, column=2, value=row.get('market', ''))
        ws.cell(row=idx, column=3, value=row.get('status', ''))
        ws.cell(row=idx, column=4, value=str(row.get('text', ''))[:80])
        ws.cell(row=idx, column=5, value=row.get('author', ''))
        ws.cell(row=idx, column=6, value=f"{row.get('age_hours', 0):.1f}h")
        ws.cell(row=idx, column=7, value=int(row.get('momentum_score', 0)))
        ws.cell(row=idx, column=8, value=round(row.get('shares_per_hour', 0), 1))
        ws.cell(row=idx, column=9, value=int(row.get('views_per_hour', 0)))
...
def create_audio_sheet(ws, df):
    ...
    for idx, (_, row) in enumerate(df.iterrows(), 2):
        ...
        ws.cell(row=idx, column=3, value=str(row.get('music_name', ''))[:50])
        ws.cell(row=idx, column=4, value=str(row.get('artist', ''))[:30])
        ws.cell(row=idx, column=5, value='Original' if row.get('is_original', False) else 'Sound')
        ws.cell(row=idx, column=6, value=f"{row.get('used_count', 0)} videos")
...
def create_my_performance_sheet(ws, your_posts, today):
    ...
    for idx, (_, row) in enumerate(your_posts.iterrows(), 2):
        ...
        ws.cell(row=idx, column=2, value=row.get('author', ''))
        ws.cell(row=idx, column=3, value=str(row.get('text', ''))[:60])
        ws.cell(row=idx, column=4, value=f"{row.get('age_hours', 0):.1f}h")
        ws.cell(row=idx, column=5, value=int(row.get('momentum_score', 0)))
        ws.cell(row=idx, column=6, value=row.get('status', ''))
        ws.cell(row=idx, column=7, value=row.get('market', ''))
        ws.cell(row=idx, column=8, value=int(row.get('views_per_hour', 0)))
        ws.cell(row=idx, column=9, value=round(row.get('shares_per_hour', 0), 1))
        ...
        ws.cell(row=idx, column=14, value=row.get('trigger_reason', ''))
...
def save_today_cache(us_df, uk_df, cache_dir):
    ...
    us_cache = us_df[['webVideoUrl', 'momentum_score']].to_dict('records') if len(us_df) > 0 else []
    uk_cache = uk_df[['webVideoUrl', 'momentum_score']].to_dict('records') if len(uk_df) > 0 else []
